...
class Anim(object):
    ...
    """
    def __init__(self, quats, pos, parents, bones):
        """
...
class AnimMapper:
    def __init__(self, skeleton, keep_translation=False, root_motion=False, match_effectors=False, effector_names=['LeftFoot', 'RightFoot'], effector_offests=None, local_offsets=None, mirror=False, mirror_left='Left', mirror_right='Right'):
    ...
    def __call__(self, anim):
        if self.match_effectors:
            anim = remap_animation_to_asset_with_ik(anim, self.skeleton, self.effector_names, self.effector_offests, self.local_offsets)
        else:
            anim = remap_animation_to_asset(anim, self.skeleton, self.keep_translation)
...
def remap_animation_to_asset(anim, skeleton, keep_translation=False):
    """
    Create a new animation that match the skeleton of the asset

    :param anim: the animation to align
    :param skeleton: the skeleton to map to
    ...
    pos = skeleton.pos[np.newaxis,...].repeat(frame_count, axis=0)

    for i, name in enumerate(names):
        if name in skeleton.names:
            bone_id = skeleton.names.index(name)
            quats[:, bone_id, :] = rotations[:, i, :]
            if i <= 1 or keep_translation:
                pos[:, bone_id, :] = positions[:, i, :]

    return Anim(quats, pos, skeleton.parents, skeleton.names)
...
def remap_animation_to_asset_with_ik(anim, asset, effector_names=['LeftFoot', 'RightFoot'], effector_offests=None, local_offsets=None ):
    ...
    return Anim(q, p, asset._mesh.bone_parents, asset._mesh.bone_names)
...
def compute_root_motion(anim):
    ...
    return Anim(rotations, positions, anim.parents, anim.bones)